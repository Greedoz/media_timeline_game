...
import { skipCard } from "@/lib/store";
import type { SkipAction } from "@media-timeline-game/shared-types";
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const body = (await request.json()) as Partial<SkipAction>;
    const invalid = validateActionBodyBase(body);
    if (invalid) return invalid;
    const snapshot = skipCard(id, {
      type: "skip",
      gameId: id,
